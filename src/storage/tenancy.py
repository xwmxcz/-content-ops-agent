...
from sqlalchemy import ForeignKey, String, Table, event, inspect, select
...
def _local_table(mapper: object) -> Table:
    """The mapped Table, narrowed from ``local_table``'s wider FromClause type."""
    table = getattr(mapper, "local_table", None)
    if not isinstance(table, Table):
        raise TenantAccessError("Mapped class does not map to a single table")
    return table


def _column_key(column: object) -> str:
    """The attribute name for a mapped column, which is always set in practice.

    ``Column.key`` is Optional because a Column exists before it is attached to
    a Table. Columns reached through ``mapper.primary_key`` are attached, so a
    None here would mean the mapper is malformed.
    """
    key = getattr(column, "key", None)
    if not isinstance(key, str):
        raise TenantAccessError("Mapped column has no attribute key")
    return key


def _state(item: object) -> InstanceState[object]:
    """Narrow ``inspect(item)`` to an InstanceState of a mapped, persistent object.

    ``sqlalchemy.inspect`` is overloaded across mappers, engines, sessions and
    instances, so its return type is a wide union that has neither ``.mapper``
    nor ``.attrs``. Every call site in this module is reached only with a mapped
    instance, so narrowing once here keeps the ORM inspection honest instead of
    scattering ignores.
    """
    state = inspect(item)
    if not isinstance(state, InstanceState):
        raise TenantAccessError("Tenant hooks require a mapped ORM instance")
    return state
...
def _validate_owned_writes(session, flush_context, instances):
    ...
    for item in owned:
        ...
        if session.user_id is not None:
            ...
            if item in session.new:
                ...
                if all(getattr(item, _column_key(column)) is not None for column in primary_key):
                    # `local_table` is typed as FromClause, which has no
                    # `.metadata`; these models map to real tables, never joins.
                    table = state.mapper.local_table
                    owner = (
                        session.connection()
                        .execute(
                            select(table.c.user_id).where(
                                *(column == getattr(item, _column_key(column)) for column in primary_key)
                            )
                        )
                        .scalar_one_or_none()
                    )
            ...
            if item not in session.new and any(
                state.attrs[_column_key(column)].history.has_changes() for column in state.mapper.primary_key
            ):
    ...
    for item in owned:
        ...
        for key, (table_name, target_key, allow_missing) in _reference_columns(state.mapper).items():
            ...
            if value is None:
                continue
            target = _local_table(state.mapper).metadata.tables[table_name]
            ...
            pending = next(
                (
                    ...
                    and _local_table(_state(candidate).mapper).name == table_name
