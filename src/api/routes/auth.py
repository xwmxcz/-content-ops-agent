...
def register(
    payload: RegisterRequest, request: Request, response: Response, accounts: AccountStore = Depends(get_account_store)
) -> SessionResponse:
...
def login(
    payload: LoginRequest, request: Request, response: Response, accounts: AccountStore = Depends(get_account_store)
) -> SessionResponse:
    _require_signing_key()
    _throttle(accounts, request, "login", 15)
    user = accounts.get_user_by_username(payload.username)
    valid = verify_password(
        user["password_hash"] if user and user["is_active"] else DUMMY_PASSWORD_HASH, payload.password
    )
...
def resource_ticket(
    payload: ResourceTicketRequest, user: dict = Depends(get_current_user), store: ContentStore = Depends(get_store)
) -> dict:
...
def logout(
    response: Response, user: dict = Depends(get_current_user), accounts: AccountStore = Depends(get_account_store)
) -> Response:
    accounts.revoke_session(user["session_id"], user["id"])
    response.delete_cookie(
        RESOURCE_SESSION_COOKIE, httponly=True, secure=config.APP_ENV == "production", samesite="strict", path="/api"
    )
