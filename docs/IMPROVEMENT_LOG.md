...
新增 `POST /api/agent/chat/stream`：同一个回合，以 SSE 边跑边报告，最后一帧 `done` 就是原接口会返回的 `ChatResponse`。

**没有复用流水线的事件表。** 动手前我说过“可以复用事件表和唤醒机制”，读完代码后改了主意：`agent_runs` 的字段是流水线专用的
（topic、content_type、style 必填），而 Chat 一轮本来就把结果落成消息。改为同一请求内直接流式返回，事件经进程内队列传递，
不新增表和迁移。

设计要点：

- **流式只是观察者。** `ChatAgentService.chat()` 多了一个可选的 `sink`；有没有它，返回值和落库内容完全一致。
  sink 抛异常只记日志，不影响回合。
- **回合独立于连接运行。** 回合是单独的 task，不在响应生成器里。浏览器关掉只会停掉进度推送，回合照常跑完并落库，
  与原 `POST /chat` 在客户端离开后的行为一致。若随连接取消，可能把一个执行到一半的写工具拦腰截断。
- **工具调用前的文字要撤回。** 模型在调用工具前流出的文字是“边想边说”，最终回复只取最后一轮，所以发 `draft_reset`，
  前端清空草稿；`done` 到达后以服务端结果为准原地替换（保持同一个 v-for key，正在读的气泡不会被重建）。
- **流式失败的处理分两种。** 一个 token 都没出就失败（网关不支持流式）→ 静默退回一次性调用；已经流出部分回复后失败 →
  直接报错，不重试，否则用户会看到两个不同的答案。
- **前端何时可以自动退回老接口。** 只有在“服务端确定没有开始这一轮”时才退回（接口 404/405，或环境读不了流式响应体）。
  网络错误、连接在 `turn_start` 之前断开都**不算**：请求可能已被接收，自动重发就是一条重复消息。与 9 月 19 日
  小红书重复发布那次是同一条原则。`turn_start` 之后断线则提示“回复仍在生成”，并切到该会话，不重发。
- `EventSource` 不能发 POST、也带不了 `Authorization` 头，所以前端用 `fetch` 读流并自己做 SSE 分帧；
  分帧处理了帧、行乃至多字节字符被网络分块切开的情况。401 的登出逻辑抽成 `handleExpiredSession()` 与 axios 共用。
- 顺带修正：Anthropic 流式（或带工具调用）的回复是“块列表”而不是字符串，原来会被 `json.dumps` 成一段 JSON 显示给用户，
  现在取其中的文本块。

验证：后端新增 12 个用例（真实 PostgreSQL），含“客户端断开后回合仍完成并落库”；前端新增 `chatStream.spec.ts`
并把 store 测试迁到流式客户端，全量 239 passed，`vue-tsc`、`npm run build` 通过。
用带延时分块的假模型起真实 API + Vite，经代理在浏览器里确认文字是逐步到达的（150 ms 采样得到 17 个不同长度）、
工具前的文字先显示后撤回、工具事件实时出现、刷新后的持久化回复与流式结果一致。
测试过程中发现并修复一处真问题：`turn_start` 之前断线会把原始的 `TypeError` 抛给界面。

未做 / 残余：

- 没有用真实 LLM 供应商验证（本机无 API key）；各家网关的流式工具调用分块格式靠 LangChain 归并，未逐家实测。
- `turn_start` 之后断线，前端只提示并切到该会话，不会自动轮询等回复出现，需要用户手动刷新。
- Chat 回合里的同步 store 调用仍在事件循环上，与原接口相同，本轮未动。
- 规划器那次模型调用没有流式（它输出的是 JSON，不是给用户看的文字）。
...
以及对话助手的效果分析工具都读它，但产品里没有任何写入路径（只有 `examples/seed_demo_data.py`
和测试会写），真实使用中所有“效果分析”都是基于空表算出来的。

- **API**：`GET/PUT /api/content/{id}/metrics`（按平台 upsert）、`POST /api/content/metrics/import`
  （单次 ≤500 行，不属于当前工作区的内容跳过并在 `missing_content_ids` 中报告，不让一个过期 id 否决整张表）。
- **语义**：平台后台给的是累计值，所以新读数**覆盖**同平台旧记录而不是追加；分析侧按“每条内容取最高浏览的一行”，
  若追加则用户向下修正的数字永远赢不了旧的错误值。写入时顺带折叠历史遗留的重复行。
- **并发**：表上没有 `(content_id, platform)` 唯一约束，因此写入前对内容行 `SELECT … FOR UPDATE` 串行化。
  并发测试先人为拉宽竞态窗口：去掉行锁时稳定产生 6 行，有锁时 1 行（原始窗口太窄，朴素写法的测试不具区分度）。
- **校验**：计数限制在 int32 可容纳范围内，平台名限制字符集；跨工作区访问一律 404 且不产生任何写入。
- **前端**：内容库详情新增“效果数据”面板（空白数字不会被当成 0 保存；同平台再次保存替换而非重复；
  慢响应不会覆盖已切换的内容），以及 CSV/表格粘贴导入对话框（浏览器端逐行校验并带行号报错，
  含千分位分隔符这类会被悄悄截断成错误数字的情况；全部通过才允许提交）。

验证：后端新增 15 个用例（真实 PostgreSQL）；前端新增 2 个测试文件，全量 219 passed，`vue-tsc`、`npm run build` 通过；
本地起 API + Vite，用 Playwright 在真实浏览器走通录入 → 覆盖 → 导入 → 面板原地刷新，桌面与 390px 视口无横向溢出。
未做：未新增迁移（刻意不加唯一约束，避免本轮引入 schema 变更）；没有从平台自动抓取数据的通道，仍需人工录入或导入。
