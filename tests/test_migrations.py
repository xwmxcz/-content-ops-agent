...
def test_alembic_upgrades_empty_postgres_to_current_schema(pg_engine):
    ...
    try:
        ...
        assert "next_event_seq" in {column["name"] for column in inspector.get_columns("agent_runs")}
        ...
        action_indexes = {tuple(index.get("column_names") or []) for index in inspector.get_indexes("proposed_actions")}
        ...
        ledger_indexes = {
            tuple(index.get("column_names") or []) for index in inspector.get_indexes("idempotency_records")
        }
        ...
        step_indexes = {tuple(index.get("column_names") or []) for index in inspector.get_indexes("run_steps")}
        assert ("run_id", "status") in step_indexes
        with pg_engine.connect() as connection:
            assert MigrationContext.configure(connection).get_current_revision() == "0010_agent_run_request"
...
def test_legacy_stamped_schema_receives_missing_runtime_ddl_columns(pg_engine):
    ...
    try:
        ...
        command.upgrade(cfg, "head")
        inspector = inspect(pg_engine)
        assert {"intent", "plan"} <= {column["name"] for column in inspector.get_columns("agent_messages")}
        assert {"pinned", "archived", "title_pinned"} <= {
            column["name"] for column in inspector.get_columns("agent_threads")
        }
        assert {"token_usage", "cost_estimate"} <= {column["name"] for column in inspector.get_columns("jobs")}
        thread_indexes = {tuple(index.get("column_names") or []) for index in inspector.get_indexes("agent_threads")}
...
def test_head_revision_with_type_nullability_and_fk_drift_fails_validation(pg_engine):
    ...
    try:
        command.upgrade(cfg, "head")
        with pg_engine.begin() as connection:
            connection.execute(text("ALTER TABLE contents ALTER COLUMN content TYPE VARCHAR(5)"))
            connection.execute(text("ALTER TABLE contents ALTER COLUMN content DROP NOT NULL"))
            connection.execute(text("ALTER TABLE calendar_events DROP CONSTRAINT calendar_events_content_id_fkey"))
        try:
            assert_schema_current(pg_engine)
...
def test_head_revision_with_schema_drift_fails_validation(pg_engine):
    ...
    try:
        ...
        with pg_engine.begin() as connection:
            ...
            connection.execute(text("ALTER TABLE agent_run_events DROP CONSTRAINT uq_agent_run_events_run_seq"))
        try:
            assert_schema_current(pg_engine)
        except SchemaVersionError as exc:
            ...
            with pg_engine.begin() as connection:
                connection.execute(
                    text("ALTER TABLE agent_run_events ADD CONSTRAINT uq_agent_run_events_run_seq UNIQUE (run_id, seq)")
                )
        else:
