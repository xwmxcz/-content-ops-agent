...
def test_https_enforcement_rejects_spoofed_forwarded_proto_from_untrusted_client(monkeypatch):
    ...
    assert client.get("/api/private", headers={"X-Forwarded-Proto": "https"}).status_code == 426
    ...
    assert trusted_proxy.get("/api/private", headers={"X-Forwarded-Proto": "https"}).status_code == 200
...
def test_frontend_proxy_logs_strip_query_strings_and_compose_is_loopback_only():
    ...
    proxy_pass = next(line.strip() for line in nginx.splitlines() if line.strip().startswith("proxy_pass "))
