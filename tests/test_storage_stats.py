...
def test_aggregate_performance_returns_grouped_stats():
    db_path = Path("data/test_aggregate_performance.db")
    if db_path.exists():
        db_path.unlink()
    store = ContentStore(db_path=str(db_path))
    try:
...
def test_aggregate_performance_handles_empty_window():
    db_path = Path("data/test_aggregate_empty.db")
    if db_path.exists():
        db_path.unlink()
    store = ContentStore(db_path=str(db_path))
    try:
...
def test_get_calendar_conflicts_returns_overlapping_events():
    db_path = Path("data/test_calendar_conflicts.db")
    if db_path.exists():
        db_path.unlink()
    store = ContentStore(db_path=str(db_path))
    try:
...
def test_list_optimization_candidates_underperforming():
    db_path = Path("data/test_optim_underperforming.db")
    if db_path.exists():
        db_path.unlink()
    store = ContentStore(db_path=str(db_path))
    try:
...
def test_list_optimization_candidates_old_drafts():
    db_path = Path("data/test_optim_old_drafts.db")
    if db_path.exists():
        db_path.unlink()
    store = ContentStore(db_path=str(db_path))
    try:
